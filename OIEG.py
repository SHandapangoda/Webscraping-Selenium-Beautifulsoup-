...
def get_url():
    ...
    mother_url = [
        "https://www.oxfordinternational.com/search?universities%5B%5D=5569&search=&study_levels%5B%5D=1207&study_levels%5B%5D=611&study_levels%5B%5D=584&page=1",
        "https://www.oxfordinternational.com/search?universities%5B%5D=5569&search=&study_levels%5B%5D=1207&study_levels%5B%5D=611&study_levels%5B%5D=584&page=2",
        "https://www.oxfordinternational.com/search?universities%5B%5D=5569&search=&study_levels%5B%5D=1207&study_levels%5B%5D=611&study_levels%5B%5D=584&page=3",
        "https://www.oxfordinternational.com/search?universities%5B%5D=5569&search=&study_levels%5B%5D=1207&study_levels%5B%5D=611&study_levels%5B%5D=584&page=4",
        "https://www.oxfordinternational.com/search?universities%5B%5D=5569&search=&study_levels%5B%5D=1207&study_levels%5B%5D=611&study_levels%5B%5D=584&page=5"]
    matching_urls = []
...
def get_ref_url():
    ...
    with open('oieglinks08-05.txt', 'r') as file:
        for url in file:
            urls.append(url)
        ...
        try:
            ...
            for fee in fees:
                ...
                if text.startswith("INT:"):
                    ...
                    int_fee_text = text.split("INT:")[1].strip().replace('FT', '').strip().replace('£',
                                                                                                   '').strip().replace(
                        ',', '').strip()
                    break
            if int_fee_text:
                ...
                for fee in fees:
                    ...
                    if text.startswith("5 semesters:") or text.startswith("4 semesters:"):
                        int_fee_text = text.split("5 semesters:")[1].strip().replace('FT', '').strip().replace('£',
                                                                                                               '').strip().replace(
                            ',', '').strip()
                        print(url, int_fee_text)
            for div in entry:
                title = div.find('h3').text.strip()
                content_paragraphs = div.find('div', class_='wysiwyg')
                if title == 'Entry requirements':
                    content = "\n".join([paragraph.text.strip() for paragraph in content_paragraphs])
            data.append(
                {'Main_URL': url, 'Name': name[0].text_content().strip(), 'Fees': int_fee_text, 'Entry': content})
        except requests.RequestException as e:
            print(f"Error fetching URL {url}: {str(e)}")
    # print(urls)
    df = pd.DataFrame(data)
    # rename the file when running (Main uni details)
    df.to_json('main_2024-08-05.json')
    df.to_excel("main_2024-08-05.xlsx")
    return df
#**************************************************************************************************************
...
def get_details():
    ...
    for url in urls:
        try:
            ...
            print(f"Error processing URL {url}: {str(e)}")
    df = pd.DataFrame(data)
    df.to_json('OIEG_2024-08-05.json')
    df.to_excel('OIEG_2024-08-05.xlsx')
    print(df)
    return df
...
def check_missing():
    new_urls = get_url()
    urls_file_path = 'urls'  # give the existing system files with URLs
